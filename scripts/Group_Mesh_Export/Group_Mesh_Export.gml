...
function export_obj(base_filename, mesh, title = "Generated by DDD/Penguin: https://dragonite.itch.io/penguin") {
    ...
    for (var i = 0; i < array_length(mesh.submeshes); i++) {
        ...
        if (submesh.tex_normal != NULL) buffer_write(output_mtl, buffer_text, "map_Kn " + string_filename(guid_get(submesh.tex_normal).name) + ".png\r\n");
        ...
        if (submesh.tex_normal != NULL) guid_get(submesh.tex_normal).SaveToFile(path + string_filename(guid_get(submesh.tex_normal).name) + ".png");
