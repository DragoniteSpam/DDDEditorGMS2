...
function export_derg(base_filename, mesh_array, format_type) {
    static HEADER = "dergxmachina";
    static VERSION = 1;
    
    var buffer = buffer_create(163384, buffer_grow, 1);
    buffer_write(buffer, buffer_string, HEADER);
...
function export_obj(base_filename, mesh, title = "Generated by DDD/Penguin: https://dragonite.itch.io/penguin") {
    ...
    for (var i = 0; i < array_length(mesh.submeshes); i++) {
        ...
        if (submesh.tex_base != NULL) guid_get(submesh.tex_base).SaveToFile(path + string_filename(guid_get(submesh.tex_base).name) + ".png");
        if (submesh.tex_ambient != NULL) guid_get(submesh.tex_ambient).SaveToFile(path + string_filename(guid_get(submesh.tex_ambient).name) + ".png");
        if (submesh.tex_specular_color != NULL) guid_get(submesh.tex_specular_color).SaveToFile(path + string_filename(guid_get(submesh.tex_specular_color).name) + ".png");
        if (submesh.tex_specular_highlight != NULL) guid_get(submesh.tex_specular_highlight).SaveToFile(path + string_filename(guid_get(submesh.tex_specular_highlight).name) + ".png");
        if (submesh.tex_alpha != NULL) guid_get(submesh.tex_alpha).SaveToFile(path + string_filename(guid_get(submesh.tex_alpha).name) + ".png");
        if (submesh.tex_bump != NULL) guid_get(submesh.tex_bump).SaveToFile(path + string_filename(guid_get(submesh.tex_bump).name) + ".png");
        if (submesh.tex_displacement != NULL) guid_get(submesh.tex_displacement).SaveToFile(path + string_filename(guid_get(submesh.tex_displacement).name) + ".png");
        if (submesh.tex_stencil != NULL) guid_get(submesh.tex_stencil).SaveToFile(path + string_filename(guid_get(submesh.tex_stencil).name) + ".png");
    }
    #endregion
...
function export_assimp(base_filename, mesh, extension) {
    if (directory_exists(PATH_ASSIMP_CONVERSION)) directory_destroy(PATH_ASSIMP_CONVERSION);
    directory_create(PATH_ASSIMP_CONVERSION);
    
    var temp_filename = PROJECT_PATH_ROOT + PATH_ASSIMP_CONVERSION + "/" + filename_change_ext(filename_name(base_filename), ".obj");
    export_obj(temp_filename, mesh);
    
    switch (extension) {
        case "fbx": assops_convert_fbx(temp_filename, base_filename); break;
        case "dae": assops_convert_dae(temp_filename, base_filename); break;
        case "stl": assops_convert_stl(temp_filename, base_filename); break;
        case "ply": assops_convert_ply(temp_filename, base_filename); break;
        case "plyb": assops_convert_plyb(temp_filename, base_filename); break;
        case "3ds": assops_convert_3ds(temp_filename, base_filename); break;
        case "gltf": assops_convert_gltf(temp_filename, base_filename); break;
        case "glb": assops_convert_glb(temp_filename, base_filename); break;
    }
}

function export_fbx(base_filename, mesh) {
    export_assimp(base_filename, mesh, "fbx");
}

function export_dae(base_filename, mesh) {
    export_assimp(base_filename, mesh, "dae");
}

function export_stl(base_filename, mesh) {
    export_assimp(base_filename, mesh, "stl");
}

function export_ply(base_filename, mesh) {
    export_assimp(base_filename, mesh, "ply");
}

function export_plyb(base_filename, mesh) {
    export_assimp(base_filename, mesh, "plyb");
}

function export_3ds(base_filename, mesh) {
    export_assimp(base_filename, mesh, "3ds");
}

function export_gltf(base_filename, mesh) {
    export_assimp(base_filename, mesh, "gltf");
}

function export_glb(base_filename, mesh) {
    export_assimp(base_filename, mesh, "glb");
}