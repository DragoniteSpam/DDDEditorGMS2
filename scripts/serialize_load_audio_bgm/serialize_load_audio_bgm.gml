...
for (var i = 0; i < n_bgm; i++) {
    ...
    if (length == 0) {
        debug("Audio file not embedded, you probably want to re-load: " + string(bgm.GUID) + " [" + bgm.name + "]");
    } else {
        ...
        FMODGMS_Snd_Set_LoopPoints(bgm.fmod, bgm.loop_start * AUDIO_FREQUENCY, bgm.loop_end * AUDIO_FREQUENCY);
    }
