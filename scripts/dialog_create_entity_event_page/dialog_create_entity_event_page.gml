...
function dialog_create_entity_event_page(argument0) {
    ...
    var el_trigger = create_list(c2 + 16, yy, "Trigger Method(s)", "<please define some>", ew, eh, 8, uivc_entity_event_trigger_method, true, dg, Game.event_triggers);
    el_trigger.select_toggle = true;
    dg.el_trigger = el_trigger;
    
    for (var i = 0; i < array_length(Game.event_triggers); i++) {
        if (page.trigger & (1 << i)) {
