...
for (var i = 0; i < n_meshes; i++) {
    ...
    while (buffer_tell(mesh.buffer) < buffer_get_size(mesh.buffer)) {
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32));   // x
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32));   // y
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32));   // z
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32));   // nx
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32));   // ny
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32));   // nz
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32) / TILESET_TEXTURE_WIDTH);   // xtex
        buffer_write(buffer, buffer_f32, buffer_read(mesh.buffer, buffer_f32) / TILESET_TEXTURE_HEIGHT);   // ytex
        var color = buffer_read(mesh.buffer, buffer_u32);
        buffer_write(buffer, buffer_f32, color & 0xffffff);   // color
        buffer_write(buffer, buffer_f32, (color & 0xff000000) >> 24);   // alpha
