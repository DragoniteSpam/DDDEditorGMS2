function MeshSubmesh(source) constructor {
    ...
    self.BakeLighting = function(light, light_color, light_ambient) {
        ...
        meshops_set_baked_lighting_params(-light.x, -light.y, -light.z, light_color, light_ambient);
        meshops_bake_lighting(buffer_get_address(self.buffer), buffer_get_size(self.buffer));
        self.internalSetVertexBuffer();
        if (self.reflect_vbuffer) {
            var op_color_amt = clamp((Settings.mesh.reflect_color >> 24) / 0xff, 0, 1);
            var op_color_value = Settings.mesh.reflect_color & 0xffffff;
            meshops_bake_lighting(buffer_get_address(self.reflect_buffer), buffer_get_size(self.reflect_buffer));
            meshops_blend_color(buffer_get_address(self.reflect_buffer), buffer_get_size(self.reflect_buffer), op_color_value, op_color_amt);
            self.internalSetReflectVertexBuffer();
        }
    };
