...
/// @param x
/// @param y
function ui_render_animation_keyframe_translate_y(argument0, argument1, argument2) {
    ...
    if (animation && (timeline_layer + 1)) {
        ...
        if (!ui_is_active(input)) {
            input.value = string(animation_get_tween_translate_y(animation, timeline_layer, floor(timeline.playing_moment)) - (rel_current ? rel_current.yy : 0));
        }
    }

    // we must abuse truthiness wherever possible
    input.interactive = !!keyframe;
    input.root.tween_translate_y.interactive = input.interactive;
