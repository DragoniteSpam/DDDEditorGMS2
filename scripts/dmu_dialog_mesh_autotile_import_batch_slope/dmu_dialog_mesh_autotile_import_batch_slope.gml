/// @param UIButton
function dmu_dialog_mesh_autotile_import_batch_slope(argument0) {
	...
	for (var i = 0; i < array_length_1d(map_contents.mesh_autotiles_slope); i++) {
	    ...
	    if (file_exists(fn)) {
	        ...
	        map_contents.mesh_autotiles_slope_raw[i] = vbuffer;
        
	        if (vbuffer) {
	            map_contents.mesh_autotiles_slope_raw[i] = buffer_create_from_vertex_buffer(vbuffer, buffer_fixed, 1);
