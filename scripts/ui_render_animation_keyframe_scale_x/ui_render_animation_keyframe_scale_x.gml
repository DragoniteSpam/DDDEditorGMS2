...
/// @param x
/// @param y
function ui_render_animation_keyframe_scale_x(argument0, argument1, argument2) {
    ...
    if (animation && (timeline_layer + 1)) {
        ...
        if (!ui_is_active(input)) {
            ui_input_set_value(input, string(animation_get_tween_scale_x(animation, timeline_layer, floor(timeline.playing_moment)) / (rel_current ? rel_current.xscale : 1)));
        }
    }
