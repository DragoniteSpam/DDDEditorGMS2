/// @param UIInput
function uivc_input_audio_loop_start(argument0) {
    ...
    if (selection + 1) {
        if (!ds_list_empty(Stuff.all_bgm)) {
            ...
            bgm.loop_start = real(input.value) * bgm.fmod_rate;
            var position = FMODGMS_Chan_Get_Position(bgm.fmod);
        
            FMODGMS_Snd_Set_LoopPoints(bgm.fmod, bgm.loop_start, bgm.loop_end);
            // setting a loop point while the sound is playing makes bgms weird so we just stop it instead
