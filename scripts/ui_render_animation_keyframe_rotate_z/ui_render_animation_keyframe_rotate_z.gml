...
/// @param x
/// @param y
function ui_render_animation_keyframe_rotate_z(argument0, argument1, argument2) {
    ...
    if (animation && (timeline_layer + 1)) {
        ...
        if (!ui_is_active(input)) {
            input.value = string(animation_get_tween_rotate_z(animation, timeline_layer, floor(timeline.playing_moment)) - (rel_current ? rel_current.zrot : 0));
        }
    }

    // we must abuse truthiness wherever possible
    input.interactive = !!keyframe;
    input.root.tween_rotate_z.interactive = input.interactive;
