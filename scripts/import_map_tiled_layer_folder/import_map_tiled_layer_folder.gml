...
if (group_visible) {
    for (var i = 0; i < ds_list_size(group_layers); i++) {
        ...
        switch (layer_type) {
            case "group": z = import_map_tiled_layer_folder(layer_data, columns, z, alpha * group_alpha, xx, yy, object_cache); break;
            case "objectgroup": z = import_map_tiled_layer_object(layer_data, columns, z, alpha * group_alpha, xx, yy, object_cache); break;
            case "tilelayer": z = import_map_tiled_layer_tile(layer_data, columns, z, alpha * group_alpha, xx, yy, object_cache); break;
        }
    }
}
