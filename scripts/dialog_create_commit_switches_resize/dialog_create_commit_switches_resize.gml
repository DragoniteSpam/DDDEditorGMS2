...
if (script_execute(argument0.validation, argument0.value)) {
    ...
    var count = ds_list_size(Stuff.all_global_switches);
    ...
    if (value == count) {
        ...
        for (var i = ds_list_size(Stuff.all_global_switches); i < value; i++) {
            var name = "Switch" + string(i);
            ds_list_add(Stuff.all_global_switches, [name, 0]);
