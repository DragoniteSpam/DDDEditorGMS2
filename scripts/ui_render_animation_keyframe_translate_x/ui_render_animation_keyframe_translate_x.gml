...
/// @param x
/// @param y
function ui_render_animation_keyframe_translate_x(argument0, argument1, argument2) {
    ...
    if (animation && (timeline_layer + 1)) {
        ...
        if (!ui_is_active(input)) {
            input.value = string(animation_get_tween_translate_x(animation, timeline_layer, floor(timeline.playing_moment)) - (rel_current ? rel_current.xx : 0));
        }
    }

    // we must abuse truthiness wherever possible
    input.interactive = !!keyframe;
    input.root.tween_translate_x.interactive = input.interactive;
