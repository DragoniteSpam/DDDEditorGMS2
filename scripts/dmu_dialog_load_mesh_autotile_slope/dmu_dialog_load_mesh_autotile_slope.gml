/// @param UIThing
function dmu_dialog_load_mesh_autotile_slope(argument0) {
    ...
    if (file_exists(fn)) {
        ...
        if (map_contents.mesh_autotiles_slope[thing.key]) {
            ...
            buffer_delete(map_contents.mesh_autotiles_slope_raw[thing.key]);
        }
        ...
        if (vbuffer) {
            map_contents.mesh_autotiles_slope_raw[thing.key] = buffer_create_from_vertex_buffer(map_contents.mesh_autotiles_slope[thing.key], buffer_fixed, 1);
            vertex_freeze(map_contents.mesh_autotiles_slope[thing.key]);
        } else {
            map_contents.mesh_autotiles_slope_raw[thing.key] = noone;
        }
