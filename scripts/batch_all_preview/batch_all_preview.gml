...
var existing_contents = map_container.contents;
...
if (buffer_md5(buffer, 0, buffer_get_size(buffer)) != EMPTY_BUFFER_MD5) {
	...
	buffer_read(buffer, buffer_datatype);
	serialize_load_map_contents_dynamic(buffer, map_container.version, map_container, true);
}
...
instance_destroy(map_container.contents);
map_container.contents = existing_contents;