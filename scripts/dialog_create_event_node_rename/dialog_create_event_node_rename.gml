...
function dialog_create_event_node_rename(argument0) {
    ...
    dg.suffix = "";
    var i;
    for (i = string_length(node.name); i > 0; i--) {
    ...
    var el_input = create_input(col1_x, yy, "Name:", ew, eh, function(input) {
        input.root.node.Rename(input.value + input.root.suffix);
    }, name_text, "", validate_string_event_name, 0, 1, VISIBLE_NAME_LENGTH, vx1, vy1, vx2, vy2, dg);
    yy += el_input.height;
