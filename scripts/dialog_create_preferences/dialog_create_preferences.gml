function dialog_create_preferences() {
    ...
    var el_bezier = create_input(col1_x, yy, "Bezier precision:", ew, eh, uivc_bezier_precision, Settings.config.bezier_precision, "0...16", validate_int, 1, 16, 2, vx1, vy1, vx2, vy2, dg);
    ...
    var el_tooltips = create_checkbox(col1_x, yy, "Show Tooltips", ew, eh, uivc_show_tooltips, Settings.config.tooltip, dg);
    el_tooltips.tooltip = "These thingies.";
    yy += el_tooltips.height + spacing;
    
    var el_npc_animation = create_input(col1_x, yy, "NPC speed:", ew, eh, uivc_bezier_precision, Settings.config.npc_animate_rate, "0...9", validate_int, 1, 16, 2, vx1, vy1, vx2, vy2, dg);
    el_npc_animation.tooltip = "The speed at which NPC (Pawn) entities will animate.";
    yy += el_npc_animation.height + spacing;
    
    var el_ui_color = create_color_picker(col1_x, yy, "UI Color:", ew, eh, uivc_ui_color, Settings.config.color, vx1, vy1, vx2, vy2, dg);
    ...
    var el_camera_fly = create_progress_bar(col1_x, yy, ew, eh, uivc_camera_fly_rate, 4, normalize(Settings.config.camera_fly_rate, 0, 1, 0.5, 4), dg);
    ...
    var el_alt_middle = create_checkbox(col1_x, yy, "Alternate Middle Click", ew, eh, uivc_settings_alt_middle, Settings.config.alternate_middle, dg);
    ...
    var el_focus_alpha = create_progress_bar(col1_x, yy, ew, eh, uivc_focus_alpha, 4, Settings.config.focus_alpha, dg);
    yy += el_focus_alpha.height + spacing;
    
    yy = yy_base;
    
    var el_code_ext = create_radio_array(col2_x + col1_x, yy, "Code File Extension:", ew, eh, uivc_code_extension, Settings.config.code_extension, dg);
    ...
    create_radio_array_options(el_code_ext, Settings.config.code_extension_map);
    
    yy += ui_get_radio_array_height(el_code_ext) + spacing;
    
    var el_text_ext = create_radio_array(col2_x + col1_x, yy, "Text File Extension:", ew, eh, uivc_text_extension, Settings.config.text_extension, dg);
    ...
    create_radio_array_options(el_text_ext, Settings.config.text_extension_map);
