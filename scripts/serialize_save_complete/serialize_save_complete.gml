function serialize_save_complete() {
    ...
    switch (Stuff.setting_hide_warnings[$ warn_untranslated_strings]) {
        case undefined:
            if (untranslated) {
                var dw = 560;
                var dh = 320;
                ...
                var el_confirm_as_is = create_button(dw / 2 - b_width / 2, dh - 32 - b_height / 2, "Export as-is", b_width, b_height, fa_center, function(button) {
                    if (button.root.el_remember.value) {
                        Stuff.setting_hide_warnings[$ warn_untranslated_strings] = warn_untranslated_strings_as_is;
                    }
                    serialize_save_data();
                    dialog_destroy();
                }, dg);
                var el_confirm_default = create_button(dw * 3 / 4 - b_width / 2, dh - 32 - b_height / 2, "Export defaults", b_width, b_height, fa_center, function(button) {
                    if (button.root.el_remember.value) {
                        Stuff.setting_hide_warnings[$ warn_untranslated_strings] = warn_untranslated_strings_as_default;
                    }
