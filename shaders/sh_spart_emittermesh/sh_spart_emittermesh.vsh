/*/
    ...
    This particular shader allows for particles to spawn at random positions within a mesh.
...
uniform float u_EmShapeDistrBurstID;
uniform float u_EmPtsPerStep;
uniform float u_EmSector;
uniform vec4 u_EmMeshOffset;
...
uniform float u_PtSprImgNum;
uniform float u_PtSprSpd;
...
{
    float distribution = mod(floor(u_EmShapeDistrBurstID * .25), 3.);
    if (distribution == 0.){return 1.;} //Linear distribution
    if (distribution == 1.){return R * R;} //Gaussian distribution (not exactly, but similar)
    return inversesqrt(R); //Inverse gaussian distribution (not exactly, but similar)
}
vec3 PtGetSpawnPos()
{
    float shape = mod(u_EmShapeDistrBurstID, 4.);
    vec3 randv = vec3(noise(), noise(), noise()) * 2. - 1.;
    if (shape == 3.)
    {    //Spherical if shape == 3
        float R = pow(noise(), 1./3.);
        return R * getRadialFactor(R) * vec3(normalize(randv.xy) * sqrt(1. - randv.z * randv.z), randv.z);
    }
    if (shape == 2.)
    {    //Cylindrical if shape == 2
...
vec3 PtGetPosition(float T, vec3 dir0, float spd0, vec4 PtSpd, vec3 gravVec)
{
    return T * (dir0 * (spd0 + T * (PtSpd[2] + T * PtSpd[3])) + T * gravVec);
}
vec3 PtGetDirection(float T, vec3 dir0, float spd0, vec4 PtSpd, vec3 gravVec)
{
    ...
    vec3 p1 = PtGetPosition(T-.01, dir0, spd0, PtSpd, gravVec);
    vec3 p2 = PtGetPosition(T+.01, dir0, spd0, PtSpd, gravVec);
...
float PtGetImageIndex(float T, float lifeSpan, float imgNum)
{
    float imgAni = u_PtSprImgNum - imgNum * 4.;
    float imgRandom = floor(imgAni * 0.5);
    float imgStretch = imgAni - imgRandom * 2.;
    float imgInd = imgRandom * floor(imgNum * noise());
    imgInd += floor(T * (mix(u_PtSprSpd, imgNum / lifeSpan, imgStretch)));
...
vec4 PtGetColour(float T)
{
    ...
    return mix(u_PtCol[colInd], u_PtCol[colInd+1], mix(noise(), fract(colProgress), step(1.0, u_PtColType)));
}
...
void main()
{
    ...
    //Particle lifetime
    float stream = mod(floor(u_EmShapeDistrBurstID / 12.), 2.);
    ...
    seed = vec2(u_EmShapeDistrBurstID + PtInd, PtInd);
    ...
    if ((PtTimeAlive > 0.) && (PtTimeAlive < PtLifeSpan) && (PtStartTime >= 0.) && (PtStartTime < u_EmLifeSpan))
    {
        ...
        vec3 spawnPos = (in_Colour2.xyz - .5 + in_Colour2.w * (vec3(noise(), noise(), noise()) - .5)) * u_EmMeshOffset.w + u_EmMeshOffset.xyz;
        ...
        //Texcoord and colour
        float imgNum = floor(u_PtSprImgNum * .25);
        float imgInd = PtGetImageIndex(PtTimeAlive,  PtLifeSpan, imgNum);
        v_vTexcoord = vec2((imgInd + vertCorner.x) / imgNum, vertCorner.y);
